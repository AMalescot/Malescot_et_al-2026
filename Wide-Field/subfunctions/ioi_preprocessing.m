function [SignalsFilt, FPS] = ioi_preprocessing(root, Trial, Parameters, hemoCorr, saveRaw, indivTrial)
%
%
%% Input:

% Root : directory where all the trials are
%
% Trial: trial structure containing the relevant infos:
...
%             - Parameters.HemoCorrChan = app.hemoCorrChan;
...
%             - Parameters.final_tBin: final framerate for video preview
%             - Parameters.ComputeHbO: 0 to skip, 1 to compute hbo
%             - Parameters.FilterSet: char vector defining filterset
%             - Parameters.DetrendBounds: time interval to exclude from
%                                             detrending
%             - Parameters.Discard: array containing the trial to discard
%
...
%
%% Output
% SignalsFilt: pre-processed data saved inside a structure (Real, Sham) containing:
%             - SignalsFilt.Green: reflectance of 525nm LED (Delta F/F0)
%             - SignalsFilt.Red: reflectance of 625nm LED (Delta F/F0)
%             - SignalsFilt.F2: jRGECO1a calcium activity (Delta F/F0)
%             - SignalsFilt.F2corr: hemodynamic corrected jRGECO1a calcium activity (Delta F/F0)
%             - SignalsFilt.HbO: oxy-hemoglobine variation (in uM)
% FPS: frame rate of the imaging
%
...
IndivSignalsFilt = struct('Green', cell(1,1),...
    ...
    'F2', cell(1,1), 'F2corr', cell(1,1), 'TrialType',cell(1,length(D)));

% Sort trials
if length(D)>1 % Sort trials in ascending order
...
for i = 1:length(D) % Compilation of trials
    if isempty(find(Parameters.Discard,i))
        cd(D(i).name);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ImagesClassification(pwd,pwd,Parameters.sBin,Parameters.tBin,0);
        ...
        for j = 1:nbColors %new loop because new channels created
            ...
            if Parameters.ComputeHbO == 1
                if contains(chan_name,{'green.dat','red.dat'})
        ...
        if Parameters.ComputeHbO == 1
            ...
            idx = or(contains(Illumination,'green'),contains(Illumination,'red'));
            disp("Starting HbO and HbR computation...");
            [HbO, HbR] = HemoCompute_HomeMade(folder, pwd, Parameters, Illumination(idx),0);
...
SignalsFilt = struct('Green', cell(2,1), 'Red', cell(2,1), 'HbO', cell(2,1),'HbR', cell(2,1),...
    'F2', cell(2,1), 'F2corr', cell(2,1));
...
for i = 1:nbColors 
    ...
    if i == 1
        ...
        save(fullfile(root,"FrameNumbers"),"real_nan","sham_nan",'-mat');
        ...
        ylim([0, max(nRealFrames+nShamFrames)]);
        legend("Real","Sham");
