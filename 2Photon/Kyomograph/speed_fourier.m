...
%% Description: 
% function that calculate the RBC velocity using kymograph and fourier
% transform
%% Input:
...
%  - dT_win : delta-timewindow, in seconds. Determines the time window
...
%    after first run. 
%OFFSET BY STEP SIZE SO THAT VALUE AT 0.2 = mean(0 : 0.2)
%% Output:
%  - Time_speed: time dimension (in s)
%  - Speed: RBC velocity (in mm/s)
%  - par: structures containing the parameters of detection
%  - X, max_freq_X and max_freq_T : user-inputed variables feeded to function
%    after first run. 
%
% Script written by Bruno-Félix Osmanski, Charpak lab for Ravi Rungta
% Modified by Éric Martineau to convert to function for integration in
...
%% delete points  %%ASK RAVI ABOUT THAT
figure;
...
Time_speed = T_frame(1:size(coef,2)) + win_N_T_anal/freq; %correction by Éric Martineau
...
mat2xls = zeros(length(Time_speed),2);
mat2xls(:,1) = Time_speed;
mat2xls(:,2) = Speed;
