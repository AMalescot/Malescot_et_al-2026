...
clc
%% Note:
% Routine to extract kymographs from multi trial acquisition.
...
%
%  Written by Eric Martineau and Antoine Malescot - Université de Montréal

%% Extract 
discardTrials = []; % Trial to discard
rbcChan = 1; % Channel with vessel labeling
...
vesselID = 'Vessel1'; %modify if multiple vessel segments in same line and run multiple times
stimOnset = 4;
...
stall = "none";% Method to exclude outliers or detect stalls:
            ...
            % "none" will skip
linebin = 1; %number of lines to bin to increase SNR
...
for k = 1:length(AllData)    
    ...
    if k == 1
        ...
        [flux, x, kmean, thresh, hThresh,fig] = rbcCount(dat,[],[],[], linerate, dT_win, dT_step, X,[]);
    else
       [~, Speed,~ ,~ ,~,~,xStall] = speed_fourier(dat, linerate, AllData(k).metadata.PixelSize, dT_win, dT_step, X, maxFreqT, maxFreqX, nStdev,stall);
       [flux, ~, ~,~,~,fig] = rbcCount(dat,kmean, thresh, hThresh, linerate, dT_win, dT_step, X,[]);
    end

    %Save flux measurements figure
